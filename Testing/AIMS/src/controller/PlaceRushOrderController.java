...
public class PlaceRushOrderController {
	...
	void requestToPlaceRushOrder() {
		...
		if (deliveryInfo.getProvince().equals("Hanoi")) {
			Map<Integer, Boolean>cartItemIDMapToRushDeliveryEligibility = cartService.checkEligibilityForRushDeliveryOfCartItems(cartItems);
			for (Map.Entry<Integer, Boolean> entry : cartItemIDMapToRushDeliveryEligibility.entrySet()) {
				// TO-DO: iterating to notify which items in user cart is eligible for rush delivery method
			}
		}
